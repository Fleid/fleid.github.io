...
To be noted that if we hadn't used the default `unittest` folder name, we could have use the `-unittestFolder` parameter of `Install-AutToolset` to pass our value.
...
We can also have a look at the output folder (`"C:\Users\fleide\Repos\ASATest\unittest\3_assert"`) to see the details of the run.
