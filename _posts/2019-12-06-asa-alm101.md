...
It is to be noted that local files are loaded all at once by the local run engine, which needs to be considered if we want to play with time logics in the query - we may have to switch to a live stream of data for more advanced windowing scenario. That sounds logical but it needs to be recognized.
