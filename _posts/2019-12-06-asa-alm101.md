...
As mentioned before, we will be using **Azure DevOps** here ([free account](https://dev.azure.com)), and in this section we'll focus on **Azure Repos**.

We won't go through the details, the plan is to first create a project (version control should be **Git**, it's the default anyway). Since I tend to reserve the base repo (the default one created with the project) for documentation, we'll create a new one specifically for our ASA project:
