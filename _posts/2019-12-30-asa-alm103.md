...
We can save the YAML file back into our repo by clicking **Save** or **Save and run** (no real difference on a starter script). It should then be located in the root folder of our repository. To get it locally we should sync the repo back in VSCode. There the YAML file can be moved into our `deploy` folder, and we can push that change back to the repo. 
...
The ASA engineering team has released a **npm package** that does exactly what the ```ASA: Compile scripts``` command do in VSCode but in command line. 

We remember that [npm](https://docs.npmjs.com/about-npm/) is a software registry, aka a package manager, supporting the node.js ecosystem. So to get our package to work in an environment we'll need to install [node.js](https://nodejs.org/en/download/) which should come with the npm command line interface.
...
As we've seen above, generating the ARM templates files from our project files requires to run the npm CLI (to install and run the package). Those templates come with null fields (credentials and secrets) that have to be updated for testing, which should be easy to do in script. After that we have to use either PowerShell or the Azure CLI to test those templates via their Azure module.

For all of those steps we could use any scripting language, including PowerShell or the Azure CLI. Those two are the main candidates here, as they have first class support in Azure DevOps.
