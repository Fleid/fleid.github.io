...
**CD for Continuous Deployment**, which is the practice of deploying our code to production automatically, once the CI has succeeded (this is a bit more complicated than that, but let's go with that). 
...
We can save the YAML file back into our repo by clicking **Save** or **Save and run** (no real difference on a starter script). It should then be located in the root folder of the repo. To get it locally we should sync the repo back in VSCode. There the YAML file can be moved into our `deploy` folder, and we can push that change back to the repo. **This should make the build pipeline unhappy** (the YAML file can't be found anymore!). Which is easy to correct in the settings, under `YAML file path`:
...
The ASA engineering team has released a npm package that does exactly as the ```ASA: Compile scripts``` command do. [Npm](https://docs.npmjs.com/about-npm/) is a software registry, aka a package manager, supporting the node.js ecosystem. 

To get our package to work in an environment we'll need to install [node.js](https://nodejs.org/en/download/) which should come with the npm command line interface.

It's [easy](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm#checking-your-version-of-npm-and-node-js) to check if both node and npm are installed, either in a PowerShell or Bash terminal:
...
Finally, we can install our ASA CI/CD package simply by going:

```PowerShell
npm install -g azure-streamanalytics-cicd
```

The `-g` switch makes the [installation global](https://docs.npmjs.com/downloading-and-installing-packages-globally), which is fitting for a tool reused across projects. That way we should be able to use the package from any location:
...
Finally, if it's good to install that package locally for local development and testing purposes, we already need to think about how it's going to be managed in Azure Pipelines. The good thing is both Node.js and npm are part of every [Microsoft-hosted agent](https://docs.microsoft.com/en-us/azure/devops/pipelines/agents/hosted?view=azure-devops#use-a-microsoft-hosted-agent) templatess (the VMs that will process our build). So we'll just need that last global installation command to get going. Let's see how to do that.

### build.ps1

Building our ASA job, generating ARM templates files from our project files, requires to run the npm CLI (to install and run the package). Then we need to update those templates to insert default values in null fields (credentials). After that we use either PowerShell or the Azure CLI to test those templates. Finally we need to move those files.

For all of those steps we could use any scripting language, bash or CLI, including either PowerShell or the Azure CLI. We use PowerShell in this article, mainly because I was having fun with it honestly. I'm sure that people that are going to be unhappy by that choice are the same ones capable of re-writing those steps in another language. To be noted that we're using [PowerShell Core](https://docs.microsoft.com/en-us/powershell/scripting/install/installing-powershell?view=powershell-6#powershell-core) whih is cross platform, if that was the blocker.
...
This script will require an environment variable: our target resource group. That variable can either be created at the build level (top right, `variables`), or in a variable group (`Library` under `Builds` and `Releases` on the right) which allow it to be shared across pipelines and linked to a key vault. We'll use a variable group. Those external variables are made available in the build agent via environment variables, the first line above maps that to an internal variable.
...
We just need to get back in **edit mode** in Azure DevOps Pipelines, and use the `Settings` link just above our step in the YAML editor, and use the wizard to pick the subscription we want to use.

![Screenshot of the fix for that connection error](https://github.com/Fleid/fleid.github.io/blob/master/_posts/201912_asa_alm101/asa_alm103_connection.png?raw=true)
...
![Screenshot of the fix for that connection error](https://github.com/Fleid/fleid.github.io/blob/master/_posts/201912_asa_alm101/asa_alm103_buildSuccess.png?raw=true)
