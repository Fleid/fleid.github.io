...
If we want to see the job run in Azure (and see output files in our target storage account), we can start the job from the Azure portal or in VSCode (Azure tab on the left, finding the job in the subscription and right-clicking to start).
...
The rest of the resources, including the shared ones, are close to free, so no real worries there. Worst case they can be deletes, since we have the scripts to rebuild them on demand.
