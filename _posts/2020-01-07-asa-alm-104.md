...
We'll name it `ASATest1KeyVault`, and put it in its own resource group, `rg-infra` for me, or at least the shared resource group, `rg-shared`, but not in the staging resource group, `rg-staging`. 

That way it will be more natural to re-use it and its secrets across environments and potentially projects (hello governance), and also make it more durable - since the point of having a staging resource group is to be able to delete and recreate it at will.
...
Note that I've already had the `authenticate` process hanging when trying to link the KeyVault in the Variable Group. I solved that by manually creating an access policy in the KeyVault for the Azure DevOps application principal (think service account) of my DevOps project with List/Get permissions on Secrets. The application principal id can be found in the Azure DevOps project **settings** (bottom left), `Service Connections` tab, selecting the right subscription then clicking on `Edit` and finally `use the full version of the service connection dialog`. It should be under `Service principal client ID`.
...
The easiest way to get started here is to go to the latest successful build, and trigger a release from there. Another option is to go straight to **Releases**, under **Builds**, and start from there.

![Screenshot of a successful build, top right is highlighted a Release button](https://github.com/Fleid/fleid.github.io/blob/master/_posts/201912_asa_alm101/asa_alm104_newRelease.png?raw=true)

Here no fancy YAML yet (at the time of writing anyway), so we'll use the "classic" experience and **start with an empty job**. Let's rename the stage to "Staging" and save.
...
![Screenshot of empty release pipeline](https://github.com/Fleid/fleid.github.io/blob/master/_posts/201912_asa_alm101/asa_alm104_emptyRelease.png?raw=true)

By default that pipeline is in "**Continuous deployment**" mode, meaning a release is created every time a new build is available. It can be changed by clicking on the little lightning next to the build icon. 
...
Finally, we need to allow the release pipeline to access the variable groups we created earlier. In **Variables** above the schema, in the **Variable groups** tab, we should `Link variable group` both for `KeyVaultVariableGroup` and `StagingVariableGroup`. 
...
I have to be honest, wiring KeyVault credentials from a variable group to a FilePath script (not inline) has been a struggle. Most of the documentation explains [how to do that for YAML mode](https://docs.microsoft.com/en-us/azure/devops/pipelines/process/variables?view=azure-devops&tabs=yaml%2Cbatch#secret-variables) (with YAML syntax) which is not generally available for releases, only builds. So I ended up just bypassing the whole topic and grabbing the credentials directly from PowerShell using `Get-AzKeyVaultSecret` (so yes, the KeyVault variable group is actually not necessary in what follows). This is a temporary workaround that should be resolved when the YAML experience finally get released.
...
$liveInputIoTHub1key = (Get-AzKeyVaultSecret -VaultName "ASATest1KeyVault" -Name "kvinputIoTHub1key").SecretValueText
$liveOutputBlobStorage1key = (Get-AzKeyVaultSecret -VaultName "ASATest1KeyVault" -Name "kvoutputBlobStorage1key").SecretValueText

# Step 2 : Update parameters with live values

$output = "$ENV:SYSTEM_ARTIFACTSDIRECTORY\$ENV:RELEASE_PRIMARYARTIFACTSOURCEALIAS\drop"

write-host "201 - Inserting live parameters"

$parametersData = Get-Content $output"\ASAtest1.JobTemplate.parameters.json" | ConvertFrom-Json
...
$parametersData | ConvertTo-Json | Out-File $output"\ASAtest1.JobTemplate.parameters.final.json"
...
New-AzResourceGroupDeployment `
  ...
  -TemplateFile $output"\ASAtest1.JobTemplate.json" `
  -TemplateParameterFile $output"\ASAtest1.JobTemplate.parameters.final.json" `
  -Mode Incremental
...
As discussed above, we had to be clever to load the KeyVault secrets. After that it's straightforward compared to our build script. Talking about `build.ps1`, now we understand why we had to include a `release.ps1` in the artifacts folder: so it can be used in the release pipeline which doesn't have access to the repository anymore.

### Azure Powershell task

The final step is to create a task in our `Staging` stage of the release pipeline to run `release.ps1`. In the stage screen we can add a new **Azure Powershell** task, linked to our Azure subscription, pointing to `release.ps1` in the artifact dropped files (easier done via the `...`), with script arguments for our credentials (not actually used, as discussed above) `-inputIoTHub1key $(kvinputIoTHub1key) -outputBlobStorage1key $(kvoutputBlobStorage1key)` using the **Latest installed version** of Powershell.

![Screenshot of the configured task](https://github.com/Fleid/fleid.github.io/blob/master/_posts/201912_asa_alm101/asa_alm104_psTask.png?raw=true)

## Success

Once everything is setup, the release pipeline should be happy and running. We can try that by updating our query (I just added a comment), commiting/pushing and observing how the change hands up in the live job. Of course this will trigger a restart of the job, but since we're restarting in `LastOutputEventTime` mode we will not loose any data (but potentially [generate duplicates](https://docs.microsoft.com/en-us/stream-analytics-query/event-delivery-guarantees-azure-stream-analytics)).

![Screenshot of the configured task](https://github.com/Fleid/fleid.github.io/blob/master/_posts/201912_asa_alm101/asa_alm104_releaseSuccess.png?raw=true)

**A final warning** on CI/CD setups and 24/7 compute resources such as an ASA job: pushing code to the origin repository will re-start them. So let's remember to turn them back off, or unplug the pipeline if need be...
