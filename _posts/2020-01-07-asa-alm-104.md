...
Once this is done, we can create a new variable group, named `KeyVaultVariableGroup`. For this one we will enable **Link secrets from an Azure keu vault as variables** and reference our newly created KeyVault from above. Once this is done, we can add variables - which means selecting which secrets from the KeyVault are going to be accessible in our pipelines.
...
**A final warning** on CI/CD setups and 24/7 compute resources such as an ASA job: pushing code to the origin repository will re-start them. So let's remember to turn them back off, or unplug the release pipeline if need be...
